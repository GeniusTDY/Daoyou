 ...
 *  CreationSession PhaseActionRegistry
 *  CreationEventBus  CreationDomainEvent WorkflowVariantPolicy
 * craftSync / craftAsync / runEventDrivenWorkflow / waitForWorkflowCompletion session
 */
export class CreationOrchestrator {
   ...
   *
   *
   * @param options.autoMaterialize -  true
   *   - true: session  phase  OUTCOME_MATERIALIZED
   *   - false: workflow  BLUEPRINT_COMPOSED
   *     `materializeOutcome(session)`  `materializeOutcomeWith()`
   * @param options.materialAnalysisMode - 'sync' 'async' LLM
   */
   ...
   *  workflow  session
   *
   *  `autoMaterialize: false` workflow  session.state.phase === BLUEPRINT_COMPOSED
   *  phase
   */
  ...
  /**
   *
   *  createSession + runEventDrivenWorkflow(materialAnalysisMode: 'sync') + waitForWorkflowCompletion
   *  CreationSession session.state.blueprint
   */
  craftSync(
  ...
  /**
   *  LLM
   *  createSession + runEventDrivenWorkflow(materialAnalysisMode: 'async') + waitForWorkflowCompletion
   *  CreationSession session.state.blueprint
   */
  craftAsync(
    input: CreationSessionInput,
  ...
  protected async enrichNamingWithLLM(session: CreationSession): Promise<void> {
    ...
    //  session.state  ProductNamingFacts
    ...
    if (enrichment) {
      ...
      model.originalName = model.name; // Task 3
