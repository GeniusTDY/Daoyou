...
export class AffixRegistry {
  ...
  private validateBoundary(def: AffixDefinition): void {
    ...
    for (const productType of def.applicableTo) {
      // Artifact  OWNER_AS_CASTER scope
      //  weapon-only
      if (
      ...
      // Skill apply_buff  OWNER_AS_CASTER listener  duration > 1
      //  buff  gongfa DOT/debuff
      if (
        productType === 'skill' &&
    ...
    // percent_damage_modifier  DAMAGE_REQUEST
